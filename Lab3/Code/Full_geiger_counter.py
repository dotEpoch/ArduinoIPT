...
def start_collection(points, dist, mean):
    try: 
        mode = "I" #input("Input data collection mode (CPI or IPC): ")
        
        if (mode == "I"):
            x = test_helper.main("meow")
            print(x)
            #ser.write('c'.encode())
            time.sleep(1)
...
def get_data():
    #-------- User Parameters ------------#
    num_points = 1 #input("Number of data points: ")
    distance =1 #input("Distance of radioactive source: ") #for naming purposes
    mean =1 #input("Mean category (HIGH, MID, LOWER): ") #for naming purposes
...
sample_number = 1 #input("Number of samples to collect: ")
